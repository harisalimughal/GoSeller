...
interface WholesalerHierarchyProps {
  wholesalerId: string;
}
...
const WholesalerHierarchy: React.FC<WholesalerHierarchyProps> = ({ wholesalerId }) => {
  ...
  const [traderSearchLoading, setTraderSearchLoading] = useState(false);
  ...
  // Auto-search traders when modal opens
  useEffect(() => {
    if (showTraderSearchModal) {
      console.log('Modal opened, searching for traders...');
      searchAvailableTraders('', 1);
      loadWholesalerProducts();
    }
  ...
  const loadHierarchyData = async () => {
    try {
      setLoading(true);
      const response = await sellerHierarchyAPI.getWholesalerHierarchy(wholesalerId);
  ...
  const searchAvailableTraders = async (search?: string, page: number = 1) => {
    try {
      setTraderSearchLoading(true);
      console.log('Searching traders with:', { wholesalerId, search, page });
      const response = await sellerHierarchyAPI.searchAvailableTraders(wholesalerId, search, page);
      console.log('Trader search response:', response);
      setAvailableTraders(response.traders);
      setTraderPagination(response.pagination);
    } catch (error) {
      console.error('Error searching traders:', error);
      alert('Failed to search traders. Please try again.');
    } finally {
      setTraderSearchLoading(false);
    }
  };

  const loadWholesalerProducts = async () => {
    try {
      setProductsLoading(true);
      const response = await sellerHierarchyAPI.getWholesalerProducts(wholesalerId);
      setWholesalerProducts(response.products);
    } catch (error) {
  ...
  const handleAddTraderFromSearch = async (data: {
    traderId: string;
    territories: string[];
    ...
    try {
      await sellerHierarchyAPI.addTraderWithProducts({
        wholesalerId,
        ...data
      });
      setShowTraderSearchModal(false);
      await loadHierarchyData();
    } catch (error) {
      console.error('Error adding trader:', error);
      alert('Failed to add trader. Please try again.');
    }
  };

  const handleRemoveTrader = async (traderId: string) => {
    if (window.confirm('Are you sure you want to remove this trader from your hierarchy?')) {
      try {
        await sellerHierarchyAPI.removeTrader(wholesalerId, traderId);
        await loadHierarchyData();
      } catch (error) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Trader Management</h1>
                <p className="text-sm text-gray-600">Manage your trader network and distribution</p>
              </div>
            </div>
            <button
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{hierarchy?.traders.length || 0}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{wholesalerProducts.length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="border-b border-gray-200">
            <nav className="flex space-x-8 px-6">
              {[
                ...
                { id: 'traders', name: 'Traders', icon: FiPackage },
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  <div className="bg-blue-50 rounded-lg p-4">
                    <h3 className="text-lg font-semibold text-blue-900">Traders</h3>
                    <p className="text-3xl font-bold text-blue-600">{hierarchy.traders.length}</p>
                    <p className="text-sm text-blue-700">Direct distributors</p>
                  </div>
                  <div className="bg-green-50 rounded-lg p-4">
                    <h3 className="text-lg font-semibold text-green-900">Storekeepers</h3>
                    <p className="text-3xl font-bold text-green-600">{hierarchy.storekeepers.length}</p>
                ...
                <div className="bg-gray-50 rounded-lg p-6">
                  ...
                  <div className="space-y-4">
                    {hierarchy.traders.slice(0, 3).map((trader) => (
                      <div key={trader._id} className="flex items-center justify-between p-4 bg-white rounded-lg">
                        <div className="flex items-center">
                          <div className="w-10 h-10 bg-orange-100 rounded-full flex items-center justify-center">
                            <FiUsers className="w-5 h-5 text-orange-600" />
                          </div>
                          <div className="ml-4">
                            <p className="font-medium text-gray-900">{trader.shopName}</p>
                        ...
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(trader.status)}`}>
                          ...
                          <span className="ml-1">{trader.status}</span>
                        </span>
                      </div>
                    ))}
                  </div>
            ...
            {activeTab === 'traders' && (
              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  <h3 className="text-lg font-semibold text-gray-900">Trader Network</h3>
                  <button
                    onClick={() => setShowTraderSearchModal(true)}
                    className="bg-orange-500 text-white px-4 py-2 rounded-lg hover:bg-orange-600 transition-colors flex items-center"
                  >
                ...
                {hierarchy?.traders.length === 0 ? (
                  <div className="text-center py-12">
                    <FiUsers className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                    <h3 className="text-lg font-medium text-gray-900 mb-2">No Traders Yet</h3>
                    <p className="text-gray-600 mb-4">Start building your trader network to expand your distribution</p>
                    <button
                      onClick={() => setShowTraderSearchModal(true)}
                    ...
                    {hierarchy?.traders.map((trader) => (
                      ...
                      >
                        <div className="flex justify-between items-start mb-4">
                          <div>
                            <h4 className="font-semibold text-gray-900">{trader.shopName}</h4>
                            <p className="text-sm text-gray-600">{trader.location}</p>
                          </div>
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(trader.status)}`}>
                            {getStatusIcon(trader.status)}
                        ...
                        <div className="space-y-2 mb-4">
                          <div className="flex items-center text-sm text-gray-600">
                            ...
                            <span>Territories: {trader.supplyChain?.authorizedTerritories?.join(', ') || 'Not assigned'}</span>
                          </div>
                          <div className="flex items-center text-sm text-gray-600">
                            <FiPackage className="mr-2" />
                            <span>Products: {trader.supplyChain?.traderConfig?.assignedProducts?.length || 0}</span>
                          </div>
                        </div>
                        ...
                        <div className="flex space-x-2">
                          ...
                          <button 
                            onClick={() => handleRemoveTrader(trader._id)}
              ...
              <div className="space-y-6">
                ...
                {productsLoading ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600">Add products to your inventory to assign to traders</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[80vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-semibold text-gray-900">Search Available Traders</h3>
              <button
            ...
            <EnhancedAddTraderModal
              traders={availableTraders}
              products={wholesalerProducts}
              loading={traderSearchLoading}
              searchTerm={traderSearchTerm}
              onSearchChange={setTraderSearchTerm}
              onSearch={() => searchAvailableTraders(traderSearchTerm, 1)}
              pagination={traderPagination}
              onPageChange={(page) => searchAvailableTraders(traderSearchTerm, page)}
              onAddTrader={handleAddTraderFromSearch}
              onClose={() => setShowTraderSearchModal(false)}
            />
          </div>
        </div>
      )}
...
// Enhanced Add Trader Modal Component
interface EnhancedAddTraderModalProps {
  traders: any[];
  products: any[];
  loading: boolean;
  searchTerm: string;
...
const EnhancedAddTraderModal: React.FC<EnhancedAddTraderModalProps> = ({
  traders,
  products,
  loading,
  searchTerm,
  ...
  const addProduct = (product: any) => {
    ...
    if (!existingProduct) {
      setSelectedProducts([...selectedProducts, {
        ...
        traderPrice: product.price * 0.85, // 15% discount for trader
  ...
  const updateProductPrice = (productId: string, price: number) => {
    setSelectedProducts(selectedProducts.map(p => 
      p.productId === productId ? { ...p, traderPrice: price } : p
    ));
  };
  ...
  const handleAddTrader = () => {
    if (selectedTrader && territories.length > 0 && selectedProducts.length > 0) {
      onAddTrader({
        ...
        <div>
          ...
          {loading ? (
            ...
            <div className="text-center py-8 text-gray-500">
              {searchTerm ? 'No traders found matching your search.' : 'No available traders found. Click "Search All Traders" to find traders.'}
            </div>
          ) : (
            <div className="space-y-4">
                  ...
                  >
                    ...
                    <div className="space-y-2 mb-3">
                      <div className="flex items-center text-sm text-gray-600">
                        ...
                        <span>Category: {trader.sellerCategory}</span>
                      </div>
                      <div className="flex items-center text-sm text-gray-600">
                        <FiClock className="mr-2" />
                        <span>Registered: {new Date(trader.createdAt).toLocaleDateString()}</span>
                      </div>
                      <div className="flex items-center text-sm text-gray-600">
                        <FiEye className="mr-2" />
                        <span>Verification: {trader.verified ? 'Verified' : 'Not Verified'}</span>
                      </div>
                    </div>
                    ...
                    >
                      {trader.status === 'approved' ? 'Add to Network' : 'Request Addition'}
                    </button>
                  </motion.div>
                ))}
              </div>
        ...
        <div>
          <div className="mb-6">
            <h4 className="text-lg font-semibold text-gray-900 mb-2">
              Add {selectedTrader?.shopName} to Your Network
            </h4>
                      ...
                      <div key={selectedProduct.productId} className="border border-gray-200 rounded-lg p-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            ...
                            <input
                              type="number"
                              value={selectedProduct.traderPrice}
          ...
          <div className="flex space-x-3">
            <button
              ...
              disabled={!selectedTrader || territories.length === 0 || selectedProducts.length === 0}
...
export default WholesalerHierarchy;
